    ...
    subSections: [
      ...
      },
      {
        ...
        topics: [
          ...
          "Network and information security threats (नेटवर्क और सूचना सुरक्षा खतरे)", 
            ...
            {section.subSections ? (
              <div className="space-y-8">
                {section.subSections.map((sub, sIdx) => (
                  <div key={sIdx}>
                    <h3 className="text-lg font-medium text-emerald-300 mb-4">{sub.subtitle}</h3>
                    <ul className="space-y-2.5">
                      {sub.topics.map((topic, tIdx) => (
                        <li 
                          key={tIdx} 
                          className="flex items-center gap-4 px-4 py-3 bg-[#0f172a]/40 hover:bg-emerald-500/5 border border-white/5 hover:border-emerald-500/20 rounded-xl text-slate-300 text-sm md:text-base transition-all duration-300 cursor-default group hover:translate-x-1"
                        >
                          <div className="flex-shrink-0 w-7 h-7 rounded-lg bg-emerald-500/10 text-emerald-400 flex items-center justify-center font-mono text-xs font-semibold group-hover:bg-emerald-500 group-hover:text-white transition-all duration-300">
